...
class MetaHyperNet(keras.layers.Layer):
    def __init__(self, dataset_cfg, model_cfg):
        super(MetaHyperNet, self).__init__()
        ...
        if dataset_cfg['USER_NUM'] == 12528:
            self.relu_func = tf.keras.layers.LeakyReLU(name="HyperRS_MetaNet_LeakyReLU")
        else:
            self.relu_func = tf.keras.layers.ReLU(name='HyperRS_MetaNet_ReLU')
        # Converter Kernel
    ...
    def call(self, user_idx):
        ...
        embedding_size = self.embedding_size
        # Converter Kernel Shape = [embedding_size, embedding_size]
...
class ItemEmbeddings(keras.layers.Layer):
    def __init__(self, dataset_cfg, model_cfg):
        ...
        # Initializers are glorot uniform. cf. Source Code of add_weight
...
class HyperRSRecommender(keras.models.Model):
    def __init__(self, dataset_cfg, model_cfg):
        super(HyperRSRecommender, self).__init__()
        self.embedding_size = model_cfg["embedding_size"]
        self.item_embedding = ItemEmbeddings(dataset_cfg, model_cfg)
        self.user_embedding = UserEmbeddings(dataset_cfg, model_cfg)
        self.hypernetwork = MetaHyperNet(dataset_cfg, model_cfg)
        self.interest_attention = tf.keras.layers.Attention(
            use_scale = False
        )
        if dataset_cfg['USER_NUM'] == 12528:
            self.relu_func = tf.keras.layers.LeakyReLU(name="HyperRS_MetaNet_LeakyReLU")
            tf.print('---------------------Detecting Book Dataset. Use LeakyReLU-------------------------')
        else:
            self.relu_func = tf.keras.layers.ReLU(name='HyperRS_MetaNet_ReLU')
    ...
    def local_update(self, grad, lr):
        ...
        # global parameters update are depending on optimizer itself.
    ...
    def call(self, user_id, genre_idx, creator_idx, character_idx, dbg=False):
        ...
        v = tf.squeeze(v)
        # tf.print("Att Item Embed Shape", att_item_embedding.shape, "Score", att_score.shape)
