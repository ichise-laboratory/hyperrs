...
from models import HyperRSRecommender
import numpy as np
import argparse
import time
import pickle
...
def mae(true_y, pred_y):
    return tf.reduce_mean(tf.math.abs(true_y - pred_y)).numpy()
...
def test_HyperRS(model, optimizer, use_dataset, dataset_user_cnt, 
    ...
    for idx, (pos_sample, neg_sample) in enumerate(use_dataset):
        ...
        tf.print(f"{idx}/{dataset_user_cnt} MAE {acc_mae/dataset_user_cnt}. \n"
...
def train_HyperRS(model, optimizer, dataset_name, basis_optimizer, train_dataset, valid_dataset, test_dataset, learning_rate,
    ...
    for epoch_num in range(EPOCH_NUM):
        for idx, (pos_sample, neg_sample) in tqdm(enumerate(train_dataset)):
            ...
            if ((idx > 0) and ( (idx + train_user_cnt * epoch_num) % eval_batch) == 0):
                test_HyperRS(model, optimizer, valid_dataset, valid_user_cnt,
                            SUPPORT_SIZE, QUERY_SIZE, "HyperRS_Valid", tf_writer, epoch_num * train_user_cnt + idx)
                test_HyperRS(model, optimizer, test_dataset, test_user_cnt,
                            SUPPORT_SIZE, QUERY_SIZE, "HyperRS_Test", tf_writer, epoch_num * train_user_cnt + idx)
...
if __name__ == "__main__":
    ...
    for k, v in tqdm(enriched_ratings.items()):
        for vv in v:
            ...
            for _id in genre_int_id_list:
                if _id > max_genre_id:
                    if _id == 0:
                        print('crap') # Oct 17 - checked. Nothing is marked as "0".
                    max_genre_id = _id
    ...
    if use_existing_items:
        ...
        if DATASET_IN_USE == 'ml-10m':
            train_user_cnt = int(len(full_user_ids) * 0.05) # otherwise it won't be any cold item.
            valid_user_cnt = int(len(full_user_ids) * 0.45)
            test_user_cnt = int(len(full_user_ids) * 0.45)
        if DATASET_IN_USE == 'Book':
            train_user_cnt = int(len(full_user_ids) * 0.05) # otherwise it won't be any cold item.
            valid_user_cnt = int(len(full_user_ids) * 0.45)
            test_user_cnt = int(len(full_user_ids) * 0.45)
        else:
    ...
    train_movie_id = set() # actually item id, not only for movies.
    ...
    if model_name == "HyperRS":
        model = HyperRSRecommender(dataset_cfg[DATASET_IN_USE], model_cfg)
        ...
        train_HyperRS(model, optimizer, DATASET_IN_USE, basis_optimizer, train_dataset, valid_dataset, test_dataset, learning_rate,
